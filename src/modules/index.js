...
class Layout extends Component {
    ...
    render() {
        ...
        <AppBar title={title} elevated={url !== '' || scrollY > HomePage.HEADER_HEIGHT} onNavIconPress={openMenu}/>
        <Navigation />
        <ScrollView scrollEventThrottle={50} ref={(c) => this._scrollView = c} onScroll={this._updateScrollY}>
          <Index />
        </ScrollView>
        {children}
      </View>);
...
export default connectTheme(connect(mapStateToProps, mapDispatchToProps)(Layout));
const styles = {
    base: {
        flex: 1,

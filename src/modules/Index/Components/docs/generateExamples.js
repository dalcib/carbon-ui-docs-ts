...
let indexImports = '';
let indexExport = '';
Object.keys(docgenOutput).map(filename => {
    const filenameParts = filename.split('/');
    let componentName = filenameParts.pop().split('.')[0];
    if (componentName === 'index')
        componentName = filenameParts.pop().split('.')[0];
    ...
    codeBlocks.forEach((codeBlock, i) => {
        fs.mkdirsSync(path.resolve(__dirname, `./${EXAMPLE_DIRNAME}/${componentName}`));
        fs.writeFileSync(path.resolve(__dirname, `./${EXAMPLE_DIRNAME}/${componentName}/${i}.js`), `${codeBlock.content}\n`);
...
//# sourceMappingURL=generateExamples.js.map
...
const parser = (source: string) => rawBuiltParser(`${source}\n\n`, { inline: false })
...
Object.keys(docgenOutput).map(filename => { // eslint-disable-line array-callback-return
  ...
  const codeBlocks = syntaxTree.filter((node: any) => node.type === 'codeBlock')
  
  if (!codeBlocks.length) return // eslint-disable-line array-callback-return
  
  indexExport += `  ${componentName}: [\n`
  
  codeBlocks.forEach((codeBlock: any, i: number) => {
    fs.mkdirsSync(
